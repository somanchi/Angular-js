<?php
   ...
   include('config.php');
   	 $acc = $_SESSION['accountnumber'];
	 	$sql1 ="SELECT * FROM complaints where accountnumber = $acc";
		$query = $db->query($sql1);
...
<table border='10'>
					...
					<th>Complaint</th>
					<th>response</th>
					<th>Complaint updated by</th>
					<th>Designation of the updater</th>
					<th>option</th>
					</tr>
				<?php
				...
				<td><?php if( $row['response'] == 'null')
					echo  "Pending";else 
					echo  $row['response'];?></td>
				<td><?php if( $row['Name'] == 'null')
					echo  "Pending";else 
					echo  $row['Name'];?></td>
				<td><?php if( $row['Designation'] == 'null')
					echo  "Pending";else 
